...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await upsertNewsArticle(input);

    return NextResponse.json({ success: true, id: result.id });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: error.issues[0]?.message || "Invalid news payload" },
        { status: 400 },
      );
    }
    ...
    console.error("[Admin News API] Failed to create news article:", error);

    return NextResponse.json(
      { error: status === 503 ? "Database is not configured" : "Failed to create news article" },
      { status },
    );
  }
}
